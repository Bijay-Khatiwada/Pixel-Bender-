...
function handleCompressionResult(file, output) {
  ...
  imageCompression(output, config.thumbnailOptions).then((thumbnailBlob) => {
    ...
    getImageDimensions(outputImageBlob, ({ width, height }) => {
      ...
      // Create preview image but DO NOT add to main DOM yet
const previewImg = document.createElement("img");
previewImg.src = outputImageBlob;
previewImg.alt = outputFileNameText;
...
previewImg.style.boxShadow = "0 0 15px rgba(0,0,0,0.5)";


// Create overlay container (initially hidden)
...
previewOverlay.style.background = "rgba(0, 0, 0, 0.5)";
previewOverlay.style.backdropFilter = "blur(6px)";
previewOverlay.style.display = "none"; // hidden initially
...
// Append overlay to body (only once per preview)
...
previewToggleBtn.style.marginBottom = "8px";
previewToggleBtn.style.cursor = "pointer";
previewToggleBtn.style.padding = "6px 12px";
previewToggleBtn.style.border = "1px solid #007bff";
previewToggleBtn.style.backgroundColor = "#007bff";
...
// Hide overlay if user clicks anywhere on overlay (including image background)
previewOverlay.addEventListener("click", () => {
  previewOverlay.style.display = "none";
});

// Prevent clicks on image from closing overlay (stop propagation)
previewImg.addEventListener("click", (e) => {
  e.stopPropagation();
});

// Build final wrapper: button + output content (no inline preview)
const finalWrapper = document.createElement("div");
finalWrapper.style.marginBottom = "1rem";
finalWrapper.appendChild(previewToggleBtn);
finalWrapper.appendChild(wrapper.firstChild);

ui.output.content.prepend(finalWrapper);
